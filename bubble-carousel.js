...
(() => {
  ...
  class BubbleCarousel {
    constructor(elementOrSelector, options = {}) {
      ...
      this.renderBubbleTooltip = options.renderBubbleTooltip || null;
    ...
    buildDOM() {
      ...
      // Tooltip container
      if (this.renderBubbleTooltip) {
        this.tooltipEl = document.createElement('div');
        this.tooltipEl.className = 'bubble-carousel-tooltip';
        this.tooltipEl.style.cssText = `
          ...
          opacity: 0;
        `;
        this.bubblesWrapper.appendChild(this.tooltipEl);
      }
    }
    ...
    rebuildBubbleElements() {
      ...
      items.forEach((item, index) => {
        ...
        if (this.renderBubbleTooltip) {
          bubbleWrapper.addEventListener('mouseenter', () => {
            const html = this.renderBubbleTooltip(item);
            if (html) {
              ...
              this.positionTooltip(bubbleWrapper);
            }
          });
      ...
      if (this.config.centerBubbleEnabled && this.config.centerBubble) {
        ...
        // Add hover tooltip listeners for center bubble
        if (this.renderBubbleTooltip) {
          bubbleWrapper.addEventListener('mouseenter', () => {
            const html = this.renderBubbleTooltip(item);
            if (html) {
    ...
    positionTooltip(targetElement) {
      if (!this.tooltipEl || !this.bubblesWrapper) return;
      const targetRect = targetElement.getBoundingClientRect();
      const wrapperRect = this.bubblesWrapper.getBoundingClientRect();
      
      const x = targetRect.left - wrapperRect.left + targetRect.width / 2;
      const y = targetRect.top - wrapperRect.top;
